...
export class AgreementTypeMasterComponent {
  ...
  validateData() {
    ...
    if (
      this.aggrementTypeDesc.length > 0 &&
      this.aggrementTypeDesc.trim().length === 0
    ) {
      vSts = false;
  ...
  saveAgreementType() {
    ...
    if (vSts) {
      ...
      if (this.aggreStDate < this.aggreEdDate) {
        ...
        this.portalServ.addAgreementType(param).subscribe(
          (res) => {
            this.ngxLoader.stop();
  ...
  deleteAgreementType(aggreTypeId: any = 0) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.ngxLoader.start();
        this.portalServ.deleteAgreementType(param).subscribe(
          (res) => {
            this.ngxLoader.stop();
  ...
  updateAgreementType() {
    ...
    if (vSts) {
      ...
      if (this.aggreStDate <= this.aggreEdDate) {
        this.ngxLoader.start();
        this.portalServ.updateAgreementType(param).subscribe(
          (res) => {
            this.ngxLoader.stop();
